...
class ProfileSummary(BaseModel):
    ...
    def describe(self) -> str:
        count_desc = str(
            self.minRowCount) if self.minRowCount == self.maxRowCount else \
            f'{self.minRowCount} - {self.maxRowCount}'
...
class BatchesSummary(BaseModel):
    ...
    def describe(self) -> str:
        count_desc = str(
            self.minBatchCount) if self.minBatchCount == self.maxBatchCount else \
            f'{self.minBatchCount} - {self.maxBatchCount}'
...
class AnalyzerDiagnosisReport(BaseModel):
    ...
    def describe(self) -> str:
        text = '\n'.join(
            [self.diagnosticData.describe(),
             self.describe_quality_issues(), self.describe_conditions()])
        return text
...
class MonitorDiagnosisReport(AnalyzerDiagnosisReport):
    ...
    def describe(self) -> str:
        text = '\n'.join(
            [self.describe_monitor(), self.describe_analyzer(), super().describe()])
        return text
